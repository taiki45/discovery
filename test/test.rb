...
Net::HTTP.start(sds_url.host, sds_url.port) do |http|
  ...
  response = http.post('/v1/registration/user', payload.map { |k,v| "#{k}=#{CGI::escape(v.to_s)}"}.join('&'))
  puts response.code, response.body
  raise_error if response.code != '200'

  payload = { ip: ab_testing_app_ip, port: 8080, revision: 'a', tags: { az: 'b', region: 'ap-northeast-1', instance_id: 'test-instance' }.to_json }
  response = http.post('/v1/registration/ab-testing', payload.map { |k,v| "#{k}=#{CGI::escape(v.to_s)}"}.join('&'))
  puts response.code, response.body
  raise_error if response.code != '200'
